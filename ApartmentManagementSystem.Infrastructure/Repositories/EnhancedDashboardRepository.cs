...
{
    /// <summary>
    /// Dashboard repository — purely read-only.
    /// No SaveChangesAsync needed here at all.
    /// Extends GenericRepository so it shares the same DbContext
    /// instance managed by UnitOfWork.
    /// </summary>
    public class EnhancedDashboardRepository
        : GenericRepository<Apartment>, IEnhancedDashboardRepository
    {
        public EnhancedDashboardRepository(AppDbContext context) : base(context) { }
        ...
        public async Task<AdvancedDashboardStatsDto> GetAdvancedDashboardStatsAsync()
        {
            var totalResidents = await DBContext.Users
                .CountAsync(u => u.UserRoles.Any(ur =>
            ...
            var communityMembers = await DBContext.Users
                .CountAsync(u => u.UserRoles.Any(ur => communityRoles.Contains(ur.Role.Name)));
        ...
        // ── Apartment portfolio stats (SuperAdmin only) ────────────
        ...
        public async Task PopulateApartmentStatsAsync(AdvancedDashboardStatsDto stats)
        {
            ...
            stats.ApartmentsUnderConstruction = apartments.Count(a => a.Status == ApartmentStatus.UnderConstruction);
            stats.TotalFloors = await DBContext.Apartments.SumAsync(a => a.TotalFloors);
            stats.TotalManagers = await DBContext.ApartmentManagers.CountAsync(m => m.IsActive);
        }

        // ── Financial (Phase 6 placeholder) ───────────────────────
        ...
        public async Task<FinancialSummaryDto> GetFinancialSummaryAsync()
        {
            var last6Months = Enumerable.Range(0, 6)
                ...
                .Reverse()
                .Select(m => new MonthlyCollectionDto
                {
        ...
        // ── Recent activity (Phase 5 placeholder) ─────────────────

        public async Task<List<RecentActivityDto>> GetRecentActivitiesAsync()
            => new List<RecentActivityDto>
            {
                new() { Activity  = "New resident registered",
                        Type      = "Registration",
                        Timestamp = DateTime.UtcNow.AddMinutes(-10) },
                new() { Activity  = "Flat assigned to resident",
                        Type      = "Assignment",
        ...
        public async Task<StaffDashboardDto> GetStaffDashboardAsync(Guid userId)
        {
            ...
            var staffMember = await DBContext.StaffMembers.FirstOrDefaultAsync(s => s.UserId == userId)
